    ...
    for call in call_logs:
        ...
        if call_tuple not in seen:
            ...
            if 'Events' in call:
                ...
                # 2) Para cada evento, deduplica participantes
                for event in call['Events']:
                    if 'Participants' in event:
                        event['Participants'] = remove_duplicates_participants(event['Participants'])
...
def remove_duplicates_participants(participants):
    # chave = PhoneNumber garante unicidade
    unique_participants = {p['PhoneNumber']: p for p in participants}
...
def clean_html(html_text):
    ...
    text = re.sub(r'<[^>]+>', '', html_text)  # Remove tags HTML
