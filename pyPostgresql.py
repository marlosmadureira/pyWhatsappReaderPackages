...
def saveResponse(telefone, unidade):
    ...
    with conectBD(DB_HOST, DB_NAME, DB_USER, DB_PASS) as con:
        ...
        sqlDadosObj = f"SELECT tbobje_intercepta.obje_id, tbobje_intercepta.linh_id FROM interceptacao.tboperadora, interceptacao.tbobje_intercepta, sistema.tbalvo, sistema.tboperacao, sistema.tbusuario, sistema.tbusuario_operacao, interceptacao.tboficio, interceptacao.tbnumerador WHERE tbobje_intercepta.opra_id  = 28 AND tboperadora.opra_id = tbobje_intercepta.opra_id AND tboficio.nume_id = tbnumerador.nume_id AND tbobje_intercepta.ofic_id = tboficio.ofic_id AND tbobje_intercepta.obje_tipo <='2'AND (tbobje_intercepta.obje_proximo is null OR tbobje_intercepta.obje_proximo in (SELECT obje_id FROM interceptacao.tbobje_intercepta WHERE obje_dtinicio is null)) AND tbobje_intercepta.ofic_id > 0 AND ( tbobje_intercepta.interceptado = 'I' AND tbobje_intercepta.obje_dtfim is null) AND tbobje_intercepta.unid_id = {unidade} AND tbalvo.alvo_id = tbobje_intercepta.alvo_id AND tbobje_intercepta.oper_id = tboperacao.oper_id AND tboperacao.oper_id = tbusuario_operacao.oper_id AND tbusuario_operacao.usua_id = tbusuario.usua_id AND ( tbobje_intercepta.obje_id IN (SELECT tbobje_intercepta.obje_id FROM interceptacao.tbobje_intercepta, linha_imei.tblinhafone, linha_imei.tbaplicativo_linhafone WHERE tbobje_intercepta.linh_id = tblinhafone.linh_id AND tblinhafone.linh_id = tbaplicativo_linhafone.linh_id AND tbaplicativo_linhafone.apli_id = 1 AND tbaplicativo_linhafone.unid_id = {unidade} AND tbaplicativo_linhafone.conta_zap ILIKE '%{telefone}%')) LIMIT 1"
